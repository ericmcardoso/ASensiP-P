  ...
  <div class="rotulo">
    ...
    <v-form @submit.prevent="onSubmit" id="check-login-form">
      <v-container>
        <v-row>
          <v-col cols="12">
            <h1>Análise de Sensibilidade - Tornado do Sistema</h1>
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12">
...
export default {
  ...
  created () {
     this.getIndicators()
     this.$store.commit("SET_TORNADOSYSTEM", "")
  },
  data() {
    return {
      errorsPresent: false,
  ...
  methods: {
    ...
    onSubmit: function() {
      if (this.$store.state.form.indicatorSelected != "" && this.isEmpty(this.$store.state.form.pSelected) != true) {
        ...
        this.searchRelation({type: 'system', model: 'tornado'});
        this.$router.push({ path: '/tornadosistema' })
      } else {
